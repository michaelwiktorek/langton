...
export class Grid {
    private readonly gridList: GridSquareColor[];

    constructor(private readonly size: number, gridList?: GridSquareColor[]) {
        if (gridList != null) {
            this.gridList = gridList;
        } else {
            const gridArea = Math.min(size * size, MAX_SIZE * MAX_SIZE);
            this.gridList = _.range(0, gridArea).map(
                num => 0 as GridSquareColor
            );
            if (size > MAX_SIZE) {
